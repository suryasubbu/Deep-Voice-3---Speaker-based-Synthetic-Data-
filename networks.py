...
def encoder(inputs, training=True, scope="encoder", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        with tf.compat.v1.variable_scope("text_embedding"):
            embedding = embed(inputs, hp.vocab_size, hp.embed_size)  # (N, Tx, e)

        with tf.compat.v1.variable_scope("encoder_prenet"):
            tensor = fc_block(embedding, hp.enc_channels, training=training) # (N, Tx, c)

        with tf.compat.v1.variable_scope("encoder_conv"):
        ...
        with tf.compat.v1.variable_scope("encoder_postnet"):
...
def decoder(inputs,
    ...
    '''
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        with tf.compat.v1.variable_scope("decoder_prenet"):
        ...
        with tf.compat.v1.variable_scope("decoder_conv_att"):
        ...
        with tf.compat.v1.variable_scope("mel_logits"):
            mel_logits = fc_block(decoder_output, hp.n_mels*hp.r, training=training)  # (N, Ty/r, n_mels*r)

        with tf.compat.v1.variable_scope("done_output"):
...
def converter(inputs, training=True, scope="converter", reuse=None):
    ...
    '''
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        with tf.compat.v1.variable_scope("converter_conv"):
        ...
        with tf.compat.v1.variable_scope("mag_logits"):

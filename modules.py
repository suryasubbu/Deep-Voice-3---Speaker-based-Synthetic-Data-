...
def conv_block(inputs,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        inputs = tf.compat.v1.layers.dropout(inputs, rate=dropout_rate, training=training)
        ...
        V = tf.compat.v1.get_variable('V',
                            ...
                            dtype=tf.float32,
                            initializer=tf.keras.initializers.variance_scaling(scale=(4.*(1.-dropout_rate)))) # (width, in_dim, out_dim)
        g = tf.compat.v1.get_variable('g',
                            dtype=tf.float32,
                            initializer=tf.norm(V.initialized_value(), axis=(0, 1), keepdims=True)
                            )
        ...
        outputs = tf.compat.v1.nn.convolution(inputs, W, padding, dilation_rate=[rate]) + b
        outputs = glu(outputs)
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        V = tf.compat.v1.get_variable('V',
                            ...
                            initializer=tf.keras.initializers.variance_scaling(
                                scale=(1. - dropout_rate))) # (in_dim, num_units)
        g = tf.compat.v1.get_variable('g',
                            dtype=tf.float32,
                            initializer=tf.norm(V.initialized_value(), axis=0, keepdims=True))
        b = tf.compat.v1.get_variable('b', shape=(num_units), dtype=tf.float32, initializer=tf.zeros_initializer)
...
def attention_block(queries,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        with tf.compat.v1.variable_scope("context"):
            ctx = tf.compat.v1.layers.dropout(alignments, rate=dropout_rate, training=training)
            ctx = tf.matmul(ctx, vals)  # (N, Ty/r, a)
            tf.to_float = lambda x: tf.cast(x, tf.float32)
            ctx *=  tf.math.rsqrt(tf.to_float(Tx))
